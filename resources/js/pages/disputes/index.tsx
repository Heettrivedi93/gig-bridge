            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                    <div className="rounded-xl border border-border/70 bg-card px-4 py-3">
                        ...
                        <p className="mt-2 text-2xl font-semibold">{disputes.length}</p>
                    </div>
                    <div className="rounded-xl border border-border/70 bg-card px-4 py-3">
                    ...
                    <div className="rounded-xl border border-border/70 bg-card px-4 py-3">
                        ...
                        <p className="mt-2 text-2xl font-semibold">{resolved}</p>
                    </div>
                </div>
                ...
                {disputes.length === 0 ? (
                    <section className="rounded-3xl border border-dashed border-border/70 bg-card px-6 py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            Disputes you raise from your orders will appear here.
                        </p>
                    </section>
                ) : (
                    <div className="overflow-hidden rounded-xl border border-border/70">
                                ...
                                <tbody className="divide-y divide-border">
                                    {disputes.map((dispute) => (
                                        <tr key={dispute.id} className="bg-background align-top hover:bg-muted/20">
                                            ...
                                            <td className="px-4 py-3">
                                                <p className="font-medium">{dispute.order_gig_title ?? '—'}</p>
                                                <p className="text-xs text-muted-foreground">Order #{dispute.order_id}</p>
                                            </td>
                                            <td className="px-4 py-3">
                        ...
                        <div className="space-y-3 p-4 lg:hidden">
                            {disputes.map((dispute) => (
                                <div key={dispute.id} className="rounded-xl border border-border/70 p-4">
                                    <div className="flex items-start justify-between gap-3">
                                        <div>
                                            <p className="font-medium">Dispute #{dispute.id}</p>
                                            <p className="text-sm text-muted-foreground">
                                                {dispute.order_gig_title ?? `Order #${dispute.order_id}`}
                                            </p>
                                        </div>
                                        <Badge variant={dispute.status === 'open' ? 'default' : 'secondary'}>
                                            {dispute.status}
                                    ...
                                    <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{dispute.reason}</p>
